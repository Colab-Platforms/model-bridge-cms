  ...
  return (
    <button
      ...
      className={`flex items-center gap-1.5 px-2 py-1 rounded text-[11px] font-medium border transition-all ${
        done ? "border-indigo-400/40 bg-indigo-500/10 text-indigo-400" : "border-white/10 text-slate-500 hover:text-slate-300"
      }`}
    >
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 bg-indigo-50 border border-indigo-100 rounded-full px-4 py-1.5 mb-5">
            ...
            <span className="text-[11px] font-black text-indigo-600 uppercase tracking-widest">New · Multi-Model Routing</span>
          </div>
          <h2 className="text-4xl md:text-5xl font-black text-[#0F172A] tracking-tight leading-tight mb-5">
            Build AI chatbots & agents<br />
            <span className="text-indigo-600">in minutes, not weeks</span>
          </h2>
          <p className="text-[17px] text-slate-500 leading-relaxed max-w-[580px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start mb-20">
          ...
          <div className="flex flex-col gap-6">
            ...
            {STEPS.map((step) => (
              <div
                key={step.num}
                className="flex items-start gap-4 bg-white border border-slate-200 rounded-2xl p-5 hover:border-indigo-200 hover:shadow-md hover:shadow-indigo-500/5 transition-all"
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[14px] font-bold text-[#0F172A] mb-1">{step.title}</p>
            ...
            <div className="flex gap-3 mt-2">
              ...
              <Link
                href="/docs"
                className="inline-flex items-center gap-2 px-6 py-3 border border-slate-200 bg-white hover:border-indigo-200 text-slate-700 text-sm font-bold rounded-2xl transition-all hover:-translate-y-0.5"
              >
          ...
          {/* Right — code */}
          <div>
            <div className="rounded-2xl overflow-hidden border border-slate-200 shadow-xl shadow-slate-900/5">
              <div className="flex items-center gap-2 px-4 py-3 bg-[#0F172A] border-b border-white/5">
                ...
                <span className="flex-1 text-center text-[11px] font-semibold text-slate-500 font-mono uppercase tracking-widest">
                  agent.ts
                </span>
            ...
            <div className="mt-4 bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Response</p>
              <div className="flex flex-col gap-2">
                {[
                  ...
                  <div key={r.model} className="flex items-center gap-3 text-[12px]">
                    ...
                    <span className="font-mono text-slate-500 w-36 truncate">{r.model}</span>
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${r.status === "success" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-600"}`}>
                      {r.status}
                    </span>
                    <span className="text-slate-400 ml-auto font-mono">{r.ms}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Use case cards */}
        <div className="mb-16">
          <p className="text-[13px] font-black text-slate-400 uppercase tracking-widest text-center mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {USE_CASES.map((uc) => (
              <div
                key={uc.title}
                className="bg-white border border-slate-200 rounded-2xl p-6 hover:border-indigo-200 hover:shadow-lg hover:shadow-indigo-500/5 hover:-translate-y-1 transition-all duration-300 group"
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center">
                    <uc.icon size={18} className="text-indigo-600" />
                  </div>
                  <span className={`text-[10px] font-bold rounded-full px-2.5 py-1 ${uc.badgeColor}`}>
                    {uc.badge}
                  </span>
                </div>
                <h3 className="text-[15px] font-bold text-[#0F172A] mb-2 group-hover:text-indigo-700 transition-colors">{uc.title}</h3>
