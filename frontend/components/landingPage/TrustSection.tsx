  ...
  return (
    <section className="bg-white dark:bg-slate-900 border-y border-slate-100 dark:border-slate-800 py-16">
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        {/* Eyebrow */}
        <p className="text-center text-[12px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-12">
          ...
          <div className="flex items-center gap-16 animate-scroll-x w-max pb-4">
            {doubled.map((p, i) => (
              ...
              >
                ...
                <span className="text-[15px] font-bold text-slate-900 dark:text-white tracking-tight">{p.name}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-16 grid grid-cols-2 lg:grid-cols-4 gap-12 text-center">
          {METRICS.map((m) => (
            <div key={m.label} className="flex flex-col items-center">
              <div className="text-[44px] font-black text-[#0F172A] dark:text-white tracking-tighter mb-1">
                {m.value}
              </div>
              <div className="text-[14px] font-bold text-slate-900 dark:text-white mb-0.5">{m.label}</div>
              <div className="text-[12px] text-slate-400 dark:text-slate-500 font-medium tracking-wide italic">{m.sub}</div>
            </div>
          ))}
        </div>
      </div>
