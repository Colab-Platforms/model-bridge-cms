...
const INTEGRATIONS: IntegrationEntry[] = [
  {
    ...
    blurb: "Cline talks to any OpenAI-compatible endpoint, so pointing it at ModelBridge is a single settings change.",
    steps: [
      "Open the Cline panel in VS Code and click the gear icon to open Settings.",
      "Under API Provider, choose OpenAI Compatible.",
      "Paste the Base URL and your ModelBridge API key below, then set a Model ID.",
      "Save — every Cline request now routes through ModelBridge.",
    ],
    blocks: [
  ...
  },
  {
    ...
    steps: [
      ...
      "Under API Provider, choose OpenAI Compatible.",
      "Paste the Base URL and your ModelBridge API key below, then set a Model ID.",
      "Save — Roo Code now routes through ModelBridge.",
    ],
    blocks: [
      { lang: "config", code: `Base URL:  ${INTEGRATIONS_BASE_URL}\nAPI Key:   mb_your_api_key\nModel ID:  gpt-4o` },
    ],
  },
  {
    ...
    steps: [
      ...
      "Under API Provider, choose OpenAI Compatible.",
      "Paste the Base URL and your ModelBridge API key below, then set a Model ID.",
      "Save — Kilo Code now routes through ModelBridge.",
    ],
    blocks: [
      { lang: "config", code: `Base URL:  ${INTEGRATIONS_BASE_URL}\nAPI Key:   mb_your_api_key\nModel ID:  gpt-4o` },
    ],
  },
  {
    ...
    steps: [
      ...
      "Paste your ModelBridge API key into the OpenAI API Key field and click Verify.",
  ...
  },
  {
    ...
    steps: [
      ...
      "Enter the Base URL, your ModelBridge API key, and the model id(s) to register.",
      "Select the new ModelBridge model from the Copilot Chat model picker.",
    ],
    blocks: [
      { lang: "config", code: `Base URL:  ${INTEGRATIONS_BASE_URL}\nAPI Key:   mb_your_api_key\nModel ID:  gpt-4o` },
    ],
  ...
  },
  {
    ...
    steps: [
      "Open ~/.codex/config.toml and add a modelbridge provider and profile (below).",
      "Export your ModelBridge key as the environment variable referenced by env_key.",
      "Run Codex with --profile modelbridge, or set it as your default profile.",
    ],
    blocks: [
      { lang: "toml", code: `[model_providers.modelbridge]\nname = "ModelBridge"\nbase_url = "${INTEGRATIONS_BASE_URL}"\nenv_key = "MODELBRIDGE_API_KEY"\n\n[profiles.modelbridge]\nmodel_provider = "modelbridge"\nmodel = "gpt-4o"` },
      { lang: "bash", code: `export MODELBRIDGE_API_KEY=mb_your_api_key\ncodex --profile modelbridge` },
    ],
  },
  {
    ...
    steps: [
      ...
      "Run gemini — requests now flow through ModelBridge.",
    ],
    blocks: [
  ...
  },
  {
    ...
    name: "Continue",
    category: "VS Code Extension",
    blurb: "Continue is config-file driven — add ModelBridge as an openai provider block in config.yaml.",
    steps: [
      "Run Continue: Open Config from the Command Palette to open config.yaml.",
      "Add a models entry with provider: openai and your ModelBridge details (below).",
      "Reload the window — ModelBridge appears in Continue's model picker.",
    ],
    blocks: [
  ...
  },
  {
    ...
    steps: [
      ...
      "Register ModelBridge as an OpenAI-compatible provider (below).",
      "Export your ModelBridge key and run opencode.",
    ],
    blocks: [
  ...
  },
  {
    ...
    steps: [
      ...
      "Save and select the new model — Droid now calls ModelBridge.",
    ],
    blocks: [
      { lang: "config", code: `Base URL:  ${INTEGRATIONS_BASE_URL}\nAPI Key:   mb_your_api_key\nModel:     gpt-4o` },
    ],
  ...
  },
  {
    ...
    note: { type: "warn", text: "This requires ModelBridge's Anthropic-compatible Messages endpoint on your deployment. If only /api/v1/chat/completions is exposed, use one of the OpenAI-compatible tools above instead." },
  },
];
        ...
        <div className="max-w-[1500px] mx-auto grid grid-cols-1 md:grid-cols-[260px_1fr] xl:grid-cols-[260px_1fr_218px]">
          ...
          <main className="min-w-0 px-6 py-8 md:px-12 md:py-12 ">
            ...
            <section className="mb-14">
              ...
              <p className="text-[15px] text-slate-600 leading-[1.75] mb-5">
                Most AI coding agents already speak the OpenAI Chat Completions format. Point their <InlineCode>base URL</InlineCode> at ModelBridge and drop in your ModelBridge API key as the <InlineCode>API key</InlineCode> — no other code changes required.
              </p>
              <Callout type="tip">
                Grab your key from the <a href="/dashboard/keys" className="text-indigo-600 font-semibold hover:underline">ModelBridge Dashboard</a> first. Every guide below uses this Base URL: <InlineCode>{INTEGRATIONS_BASE_URL}</InlineCode>
              </Callout>
